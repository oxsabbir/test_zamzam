...
const laundryPackages = [
  { id: 1, title: "Basic Wash", type: "Normal", deliveryTime: "24 Hours", price: 15, pickup: "Included", featured: false },
  { id: 2, title: "Quick Wash", type: "Normal", deliveryTime: "12 Hours", price: 20, pickup: "Included", featured: false },
  { id: 3, title: "Fresh Press", type: "Normal", deliveryTime: "8 Hours", price: 25, pickup: "Included", featured: true },
  { id: 4, title: "Express Clean", type: "Urgent", deliveryTime: "5 Hours", price: 30, pickup: "SAR 10 Extra", featured: false },
  { id: 5, title: "Rush Service", type: "Urgent", deliveryTime: "3 Hours", price: 35, pickup: "SAR 10 Extra", featured: false },
  { id: 6, title: "1-Hour Flash*", type: "Urgent", deliveryTime: "1 Hour*", price: 40, pickup: "SAR 10 Extra", featured: false, note: "*Available within central Makkah areas only" },
];

const ironingService = { title: "Professional Ironing", price: 12, deliveryTime: "Within 3 Hours", pickup: "Included" };
    ...
    <section className="py-16 sm:py-20 bg-gradient-to-br from-slate-50 via-emerald-50/40 to-teal-50/60">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 sm:mb-16">
          <Badge className="mb-4 bg-primary/10 text-primary hover:bg-primary/20 border-none px-4 py-1">Simple Pricing</Badge>
          <h2 className="text-3xl sm:text-5xl font-bold mb-4 text-foreground tracking-tight">Laundry Packages</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Wash, dry, and press by the kilogram. Pick a turnaround time that fits your schedule.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto mb-16">
          {laundryPackages.map((pkg) => (
            <div key={pkg.id} className={`group flex flex-col rounded-3xl border transition-all duration-500 hover:-translate-y-2 ${
              pkg.featured
              ...
              <div className={`p-6 rounded-t-[22px] ${
                pkg.type === "Urgent" ? "bg-gradient-to-br from-red-50 to-orange-50/50" : "bg-gradient-to-br from-primary/5 to-transparent"
              }`}>
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <Badge className={`mb-2 font-bold px-3 ${
                      pkg.type === "Urgent" ? "bg-red-500 hover:bg-red-600 text-white" : "bg-primary text-white hover:bg-primary/90"
                    }`}>{pkg.type}</Badge>
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <Button id="generate_lead" onClick={() => handleBooking(pkg.price, pkg.title, pkg.deliveryTime)}
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="text-center md:text-left">
                  ...
                  <p className="text-muted-foreground mb-4">Expert pressing and ironing service for a crisp, professional look.</p>
                  <div className="flex flex-wrap justify-center md:justify-start gap-4">
                    <div className="flex items-center text-sm font-medium bg-background/80 px-3 py-1.5 rounded-full border">
                      <Clock className="w-4 h-4 mr-2 text-primary" />{ironingService.deliveryTime}
                    </div>
                    <div className="flex items-center text-sm font-medium bg-background/80 px-3 py-1.5 rounded-full border">
                      <Truck className="w-4 h-4 mr-2 text-primary" />Pickup {ironingService.pickup}
                    </div>
                  </div>
                </div>
                <div className="text-center bg-background rounded-2xl p-6 shadow-sm border border-primary/10 min-w-[160px]">
                  <div className="text-4xl font-black text-primary mb-1">{ironingService.price}</div>
                  <div className="text-sm font-bold text-muted-foreground uppercase tracking-wider">SAR / KG</div>
                  <Button onClick={() => handleBooking(ironingService.price, "Ironing Only", ironingService.deliveryTime)} className="mt-4 w-full bg-primary text-white">
                    Book Now
                  </Button>
                </div>
              </div>
